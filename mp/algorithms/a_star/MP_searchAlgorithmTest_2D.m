...
%                 fringeNode.h = MP_getHeuristic_2D(sp.typeOfHeuristic, sp.start_conf, sp);
        ...
        greedyChildren = [];
        nextConfig = greedyExpand2D(sp, fringeNode.path(:, end - 1:end), sp.goal_conf);
        if ~isempty(nextConfig)
            greedyChildren.path = [fringeNode.path, nextConfig];
            greedyChildren.g = MP_calculateCost_2D(fringeNode.path(:, end - 1:end), nextConfig, sp.home_base);
            greedyChildren.h = getHeuristic2D(sp, nextConfig, sp.goal_conf);
            greedyChildren.f = MP_calculateCostBasedOnAlgorithm(greedyChildren.g, greedyChildren.h, sp.typeOfAlg);
        end
